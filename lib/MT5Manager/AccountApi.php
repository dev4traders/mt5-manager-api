...
class AccountApi
{
    ...
    {
        list($response) = $this->accountLoginGetWithHttpInfo($token, $login);
        return $response;
    }
     ...
     */
    public function accountLoginGetAsync($token, $login)
    {
        return $this->accountLoginGetAsyncWithHttpInfo($token, $login)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     */
    protected function accountLoginGetRequest($token, $login)
    {
        // verify the required parameter 'token' is set
        if ($token === null || (is_array($token) && count($token) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $token when calling accountLoginGet'
            );
        }
